...
const CARD_BASE_POSITION_DURATION = 1000
const CARD_BASE_ROTATION_DURATION = 300
const CARD_BASE_FLIP_DURATION = 800
const CARD_BASE_SCALE_DURATION = 300
const CARD_BASE_HIGHLIGHT_DURATION = 400
const CARD_BASE_VISIBILITY_DURATION = 400

//Idle
const IDLE_DURATION = 2000
const IDLE_RANGE = 0.5
const IDLE_LENGTH = 5

//Mouse up
const CARD_POSITION_RESET_DURATION = 600
...
export const InteractibleComponent = forwardRef<InteractibleComponentHandle, InteractibleComponentProps>((props: InteractibleComponentProps, ref: React.Ref<InteractibleComponentHandle>) => {
  ...
  const targetRestAngle = useMemo(() => {
    const percentage = Math.abs(randomOffset) / ((props.width ?? 0) * 0.35)
  ...
  useEffect(() => {
    if (backgroundRef.current) {
      ...
      if (isBlinkingRef.current && blinkTweenRef.current) {
  ...
  const toggleIdle = (isPlayingIdle: boolean) => {
    ...
    if (isPlayingIdle) {
      const animateIdle = () => {
        const range = aspectWidth(IDLE_RANGE)
        const startRange = -range / 2
        const duration = (1 + Math.random()) * IDLE_DURATION

        const newTarget = {
          x: Array.from({ length: IDLE_LENGTH }, () => startRange + Math.random() * range),
  ...
  const setComponentRotation = (rotation: number[] | number, duration = CARD_BASE_ROTATION_DURATION, easing = TWEEN.Easing.Quadratic.InOut, interpolation = TWEEN.Interpolation.Linear) => {
  ...
  const setComponentScale = (scale: number[] | number, duration = CARD_BASE_SCALE_DURATION, easing = TWEEN.Easing.Quadratic.InOut, interpolation = TWEEN.Interpolation.Linear) => {
